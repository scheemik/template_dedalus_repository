"""
Plot energy flux from joint analysis files.

Usage:
    plot_ef.py EXP_NAME <files>... [--output=<dir>]
...
def build_ef_dicts():
    advec_dict = {'ef_name':   "Kinetic Advection",
                  'ef_task':   'ef_advec',
                  'ef_ylabel':r'<$F_z(z,t)_a$> ' + ef_units}
    press_dict = {'ef_name':   "Sponge Layer",
                  'ef_task':   'ef_press',
                  'ef_ylabel':r'<$F_z(z,t)_p$> ' + ef_units}
    visc_dict  = {'ef_name':   "Rayleigh Friction",
                  'ef_task':   'ef_visc',
                  'ef_ylabel':r'<$F_z(z,t)_v$> ' + ef_units}
    return advec_dict, press_dict, visc_dict

# Sets parameters according to the switchboard settings
def flip_the_switches(plot_ef_total, plot_ef_comps, use_sst, T):
    advec_dict, press_dict, visc_dict = build_ef_dicts()
